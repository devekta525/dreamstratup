...
export default function ProductDetailsPage({
  ...
  const handleSubmitReview = async () => {
    ...
    if (!user) { toast.error('Please login to submit a review.'); router.push('/login'); return; }
    setSubmittingReview(true);
    try {
      await reviewService.createReview(id, { rating: reviewRating, comment: reviewComment.trim() });
  ...
  const hasBulkTiers = product.bulkPricingTiers && product.bulkPricingTiers.length > 0;
  const hasSpecs = product.specifications && Object.keys(product.specifications).length > 0;
  const inStock = product.stock > 0;
  const price = effectivePrice();
  const sortedTiers = hasBulkTiers
    ? [...product.bulkPricingTiers].sort((a: BulkPricingTier, b: BulkPricingTier) => a.minQty - b.minQty)
    : [];
    ...
    <div className="min-h-screen bg-[#f5f5f5] dark:bg-gray-900">
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 py-2.5">
          <nav className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
            ...
            <span className="text-gray-300">&gt;</span>
            <Link href={`/shop?category=${encodeURIComponent(product.category)}`} className="hover:text-orange-500 transition">
              {product.category}
            </Link>
            <span className="text-gray-300">&gt;</span>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            <div className="lg:col-span-5 p-4 lg:p-6 flex gap-3">
              ...
              {images.length > 1 && (
                <div className="flex flex-col items-center gap-1 w-[68px] shrink-0">
                  ...
                  {images.length > maxVisibleThumbs && (
                    <button
                      onClick={() => setThumbStart(Math.min(images.length - maxVisibleThumbs, thumbStart + 1))}
            ...
            <div className="lg:col-span-7 border-l-0 lg:border-l border-gray-200 dark:border-gray-700">
              ...
              <div className="p-4 lg:p-6 pb-3">
                ...
                <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  {product.brand && (
                    <span>Brand: <strong className="text-gray-700 dark:text-gray-200">{product.brand}</strong></span>
                  )}
                  <span>Category: <Link href={`/shop?category=${encodeURIComponent(product.category)}`} className="text-orange-500 hover:underline font-medium">{product.category}</Link></span>
                </div>
                {/* Rating summary */}
                  ...
                  <div className="flex items-center gap-0.5">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <FiStar
                        key={star}
                        size={14}
                        className={star <= Math.round(product.avgRating || 0) ? 'fill-orange-400 text-orange-400' : 'text-gray-300'}
                      />
                    ))}
                  </div>
                  <span className="text-xs font-bold text-gray-700 dark:text-gray-200">{product.avgRating?.toFixed(1) || '0.0'}</span>
                      ...
                      {sortedTiers.map((tier: BulkPricingTier, idx: number) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className={`text-xl font-extrabold ${isActive ? 'text-orange-600' : 'text-gray-800 dark:text-gray-200'}`}>
              ...
              <div className="px-4 lg:px-6 py-4 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600 dark:text-gray-300">
                    MOQ: <strong className="text-gray-900 dark:text-white">{product.moq} units</strong>
                  </span>
                  <span className="text-sm text-gray-600 dark:text-gray-300">
                    Unit Price: <strong className="text-orange-600">₹{price.toLocaleString('en-IN')}</strong>
                  </span>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">
                      Total: <strong className="text-gray-900 dark:text-white text-base">₹{(price * quantity).toLocaleString('en-IN')}</strong>
                    </div>
                  </div>
                </div>
              ...
              <div className="px-4 lg:px-6 pb-5">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 border-2 border-orange-500 text-orange-500 hover:bg-orange-500 hover:text-white disabled:border-gray-300 disabled:text-gray-300 disabled:cursor-not-allowed font-semibold py-3 px-6 rounded-full transition text-sm"
                >
                  {addingToCart ? (
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Bulk Pricing Table */}
          {hasBulkTiers && (
            <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden ${!hasSpecs ? 'lg:col-span-3' : ''}`}>
              <div className="px-5 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
                <h2 className="text-sm font-bold text-gray-800 dark:text-gray-100 uppercase tracking-wide">
                  Bulk Pricing
                </h2>
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between px-5 py-3 cursor-pointer transition ${
                        isActive ? 'bg-orange-50 dark:bg-orange-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-700/50'
                      }`}
                      onClick={() => setQuantity(tier.minQty)}
                    >
        ...
        <div className="mt-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              <div className="flex items-center gap-0.5">
                {[1, 2, 3, 4, 5].map((star) => (
                  <FiStar key={star} size={14} className={star <= Math.round(product.avgRating || 0) ? 'fill-orange-400 text-orange-400' : 'text-gray-300'} />
                ))}
              </div>
              <span className="text-sm font-bold text-gray-700 dark:text-gray-200">{product.avgRating?.toFixed(1) || '0.0'}</span>
            </div>
          </div>
          ...
          {user && user.role !== 'admin' && (
            <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-3">Write a Review</h3>
          ...
          {!user && (
            <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700 text-center">
              <p className="text-sm text-gray-500 dark:text-gray-400">
                <Link href="/login" className="text-orange-500 font-semibold hover:underline">Login</Link> to write a review.
              </p>
            </div>
          )}
                ...
                <div key={review._id} className="px-5 py-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">{review.user?.name || 'User'}</p>
                        <div className="flex items-center gap-1 mt-0.5">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <FiStar key={star} size={12} className={star <= review.rating ? 'fill-orange-400 text-orange-400' : 'text-gray-300'} />
                          ))}
                  ...
                  <p className="mt-2 text-sm text-gray-600 dark:text-gray-300 leading-relaxed">{review.comment}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="mt-6 bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg p-5 sm:p-6 text-white flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <h3 className="text-base font-bold mb-0.5">Need a custom quote or bulk deal?</h3>
