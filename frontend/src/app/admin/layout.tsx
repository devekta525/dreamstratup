...
function Sidebar({
  ...
  return (
    <aside className="relative flex h-full flex-col bg-[#060913] text-slate-300 before:absolute before:inset-0 before:bg-gradient-to-b before:from-indigo-600/10 before:to-transparent before:pointer-events-none border-r border-white/5">
      ...
      {/* Logout */}
      <div className="shrink-0 border-t border-white/5 p-4 z-10">
        <button
          onClick={handleLogout}
          className="group flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-slate-400 transition-all hover:bg-red-500/10 hover:text-red-400 cursor-pointer"
        >
          <FiLogOut size={20} className="transition-colors group-hover:text-red-400" />
          Sign Out
        </button>
      </div>
    </aside>
